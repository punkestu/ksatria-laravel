        ...
        <form action="{{ route('agenda.update', $agenda->id) }}" method="post">
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" required
                    class="p-2 mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('description') ?? $agenda->description }}</textarea>
                @if ($errors->has('description'))
                    <p class="text-red-500 text-xs mt-1">{{ $errors->first('description') ?? $agenda->description }}</p>
                @endif
            </div>
            <div class="mb-4">
